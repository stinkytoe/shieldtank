...
impl<A: Asset> LdtkComponentExt<A> for LdtkComponent<A> {
    fn is_loaded(&self, asset_server: &AssetServer) -> bool {
        asset_server.is_loaded(self.handle.id()) && asset_server.is_loaded(self.config.id())
    }
...
pub(crate) fn handle_project_component_added<A: Asset>(
    ...
    query_changed: Query<(&LdtkComponent<A>, Entity), Changed<Project>>,
) -> Result<()> {
...
pub(crate) fn send_finalize_if_ready<A: Asset>(
    ...
    awaiting_finalize.map.retain(|&entity| {
        debug!("waiting on entity: {entity:?}");
